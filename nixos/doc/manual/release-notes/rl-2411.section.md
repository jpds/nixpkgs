...
- Caddy can now be built with plugins by overriding the `newVendorHash` attribute and using `caddy.withPlugins`, a
  `passthru` function that accepts a list of attribute sets as a parameter. Each attribute set represents a Caddy plugin
  , and the valid attributes for a Caddy plugin are `name`, `repo`, and `version`.

  Example:
  ```
  services.caddy = {
    enable = true;
    package = (pkgs.caddy.override { newVendorHash = "<SHA256 hash>"; }).withPlugins
      [
  ...
  To get the necessary SHA256 hash of the vendored dependencies, set `newVendorHash = "";`. 
